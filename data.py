...
def _maybe_randaugment(randaug=False, n=2, m=9):
    if not randaug:
        return []
    else:
...
def transform_cifar(augment=False, from_tensor=False, normalize=True,
    ...
    pre_cast = []
    if augment:
        pre_cast += [transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip()]
        print("Dataset with basic Cifar augmentation")
    pre_cast += _maybe_randaugment(randaug, randaug_n, randaug_m)

    # ToTensor
    cast = [] if from_tensor else [transforms.ToTensor()]

    # Normalize
    normal_fn = [transforms.Normalize(mean=MEANS['cifar'], std=STDS['cifar'])] if normalize else []

    train_transform = transforms.Compose(pre_cast + cast + normal_fn)
    test_transform  = transforms.Compose(([] if from_tensor else [transforms.ToTensor()]) + normal_fn)
...
def transform_svhn(augment=False, from_tensor=False, normalize=True,
    ...
    pre_cast = []
    if augment:
        pre_cast += [transforms.RandomCrop(32, padding=4)]
        print("Dataset with basic SVHN augmentation")
    if randaug:
        try:
            from torchvision.transforms import RandAugment
            pre_cast.append(RandAugment(num_ops=randaug_n, magnitude=randaug_m))
            print(f"RandAugment enabled: N={randaug_n}, M={randaug_m}")
        except Exception as e:
            raise RuntimeError("当前 torchvision 不支持 RandAugment，请升级到 torchvision>=0.11") from e

    # 张量化
    if from_tensor:
        pre_cast = [transforms.ToPILImage()] + pre_cast
        cast = [transforms.ToTensor()]
    else:
        cast = [transforms.ToTensor()]

    # 归一化
    normal_fn = [transforms.Normalize(mean=MEANS['svhn'], std=STDS['svhn'])] if normalize else []
...
def transform_mnist(augment=False, from_tensor=False, normalize=True,
    ...
    pre_cast = []
    if augment:
        pre_cast += [transforms.RandomCrop(28, padding=4)]
    ...
    # 归一化
    normal_fn = [transforms.Normalize(mean=MEANS['mnist'], std=STDS['mnist'])] if normalize else []
...
def transform_fashion(augment=False, from_tensor=False, normalize=True,
    ...
    # 归一化
    normal_fn = [transforms.Normalize(mean=MEANS['fashion'], std=STDS['fashion'])] if normalize else []
...
def transform_imagenet(size=-1,
    ...
    # PIL 阶段（RRC / Flip / RandAugment）
    pre_cast = []
    if augment:
        if rrc and size >= 0:
            if rrc_size == -1:
                rrc_size = size
            pre_cast.append(transforms.RandomResizedCrop(rrc_size, scale=(0.5, 1.0)))
            print("Dataset with basic imagenet augmentation and RRC")
        else:
            print("Dataset with basic imagenet augmentation")
        pre_cast.append(transforms.RandomHorizontalFlip())
    if randaug:
        try:
            from torchvision.transforms import RandAugment
            pre_cast.append(RandAugment(num_ops=randaug_n, magnitude=randaug_m))
    ...
    # Tensor 阶段（保留你的 jittering / lighting）
    post_cast = []
    if augment:
        ...
        post_cast += [jittering, lighting]

    # 归一化
    normal_fn = [transforms.Normalize(mean=MEANS['imagenet'], std=STDS['imagenet'])] if normalize else []

    train_transform = transforms.Compose(resize_train + pre_cast + cast + post_cast + normal_fn)
    test_transform  = transforms.Compose(resize_test  + ([] if from_tensor else [transforms.ToTensor()]) + normal_fn)
    return train_transform, test_transform
...
def load_data(args, tsne=False):
    ...
    if args.dataset.startswith('cifar'):
        train_transform, test_transform = transform_cifar(augment=args.augment,
                                                        randaug=getattr(args, 'randaug', False),
                                                        randaug_n=getattr(args, 'randaug_n', 2),
                                                        randaug_m=getattr(args, 'randaug_m', 9),)
